    ...
    if ($action === 'save_appointment') {
        ...
        if (in_array($user['role'], ['admin','pharmacist','staff'])) {
            ...
            if ($id && $status) {
                ...
                $stmt->execute();
                logAudit($user['id'], "Appointment status updated to $status", 'UPDATE', 'appointments', $id, null, null, 'INFO', "Status changed to $status");
        ...
        if (in_array($user['role'], ['admin','pharmacist','staff'])) {
            ...
            if ($id) {
                ...
                $stmt->execute();
                logAudit($user['id'], 'Appointment deleted', 'DELETE', 'appointments', $id, null, null, 'WARNING', 'Appointment removed by user');
                $message = 'Appointment deleted.'; $messageType = 'success';
            }
        }
    }
}
...
$allAppts = [];
$appts = [];
$stmt = $db->prepare("SELECT a.*, CONCAT(p.first_name,' ',p.last_name) as patient_name, p.user_id as patient_user_id FROM appointments a LEFT JOIN patients p ON a.patient_id=p.id WHERE MONTH(a.appointment_date)=? AND YEAR(a.appointment_date)=? ORDER BY a.appointment_date, a.appointment_time");
...
$patients = [];
if ($user['role'] !== 'patient') {
    $stmt = $db->prepare("SELECT id, CONCAT(first_name,' ',last_name) as name FROM patients WHERE status='active' ORDER BY first_name");
    $stmt->execute();
