...
$types  = '';
if ($search) {
    $where .= " AND (action LIKE ? OR user_name LIKE ? OR audit_id LIKE ?)";
    $searchTerm = "%$search%";
    $params[] = $searchTerm;
    $params[] = $searchTerm;
    $params[] = $searchTerm;
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    $exportQuery = "SELECT audit_id, user_name, user_role, action, action_type, severity, affected_table, record_id, ip_address, notes, created_at FROM audit_logs $where ORDER BY created_at DESC";
    ...
    fputcsv($output, ['Audit ID','User','Role','Action','Type','Severity','Table','Record ID','IP Address','Notes','Timestamp']);
    while ($row = $result->fetch_assoc()) {
        fputcsv($output, [$row['audit_id'],$row['user_name'],$row['user_role'],$row['action'],$row['action_type'],$row['severity'],$row['affected_table'],$row['record_id'],$row['ip_address'],$row['notes'],$row['created_at']]);
    }
    fclose($output);
    exit;
}
...
$logs = [];
$query = "SELECT * FROM audit_logs $where ORDER BY created_at DESC LIMIT 200";
